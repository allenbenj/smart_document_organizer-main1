...
class DocumentOrganizationTab(QWidget):  # type: ignore[misc]
    ...
    def format_results_html(self, results):  # noqa: C901
        ...
        # Handle different response formats (data.results vs results vs files)
        file_list = []
        if isinstance(results, dict):
            if "results" in results and isinstance(results["results"], list):
                file_list = results["results"]
            elif "files" in results and isinstance(results["files"], list):
                file_list = results["files"]
            elif "data" in results and isinstance(results["data"], dict) and "results" in results["data"]:
                file_list = results["data"]["results"]

        # Process summary
        html.append("<h2>Document Processing Results</h2>")
        html.append(f"<p><strong>Files Processed:</strong> {len(file_list)}</p>")
        ...
        for file_result in file_list:
            ...
            if content:
                ...
                html.append(f"<pre style='white-space: pre-wrap; background: #eee; padding: 10px;'>{preview}</pre>")
