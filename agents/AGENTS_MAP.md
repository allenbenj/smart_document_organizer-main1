...
Overview
- This document provides a consolidated mapping of all agent types implemented under agents, including the concrete classes, their primary public methods, and the data contracts they expect or return. It serves as a single source of truth for how the GUI and orchestration layer interact with production, fallback, and simple agents.
...
- Core references:
  - ProductionAgentManager: [`agents/production_agent_manager.py`](agents/production_agent_manager.py:1)
  - Enhanced factory and wiring: [`agents/base/enhanced_agent_factory.py`](agents/base/enhanced_agent_factory.py:1)
  - Production service container: [`core/container/service_container_impl.py`](core/container/service_container_impl.py:1)
...
- Concrete classes and key public methods
  ...
  - Global analytics hook
    ...
    - Reference: [`agents/analytics_manager.py`](agents/analytics_manager.py:1) and its singleton getter at bottom: get_analytics_manager
...
- Core references:
  - FallbackAgentManager: [`agents/fallback_agent_manager.py`](agents/fallback_agent_manager.py:1)
...
- Core references:
  - SimpleAgentManager: [`agents/simple_agent_manager.py`](agents/simple_agent_manager.py:1)
...
- Concrete classes and key public methods
  ...
  - References:
    - [SimpleAgentManager](agents/simple_agent_manager.py:1)
...
- Core references to open-source or internal utilities:
  - Enhanced factory: [`agents/base/enhanced_agent_factory.py`](agents/base/enhanced_agent_factory.py:1)
  - Production container: [`core/container/service_container_impl.py`](core/container/service_container_impl.py:1)
  - Analytics singleton: [`agents/analytics_manager.py`](agents/analytics_manager.py:1) get_analytics_manager
...
- Notes:
  - These are not direct Python runtime classes under `agents`; they are declarative persona profiles that map into DB-backed `manager_personas` + `manager_skills`.
...
- Keep runtime class contracts (`agents`) and declarative profile contracts (`agent_resources/chatAgents`) distinct but linked.
