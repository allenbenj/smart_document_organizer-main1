...
class LegalPrecedentAnalyzer(BaseAgent, LegalMemoryMixin):
    """
    ...
    def _initialize_models(self):  # noqa: C901
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE and SentenceTransformer is not None:
            try:
                self.models["sentence_transformer"] = cast(Any, SentenceTransformer)(
                    self.config.embedding_model
                )
                logger.info(
                    f"Loaded sentence transformer: {self.config.embedding_model}"
                )
    ...
    def _initialize_citation_patterns(self):
        ...
        self.citation_patterns = {
            "case_citations": [
                ...
                CitationPattern(
                    "short_citation", r"(\\d+)\\s+([A-Za-z\\.]+)\\s+(\\d+)", "unknown"
                ),
            ],
            "statute_citations": [
                CitationPattern(
        ...
        logger.info(
            f"Initialized {sum(len(patterns) for patterns in self.citation_patterns.values())} citation patterns"
        )
    ...
    def _log_available_models(self):
        ...
        models_status = {
            ...
            and "sentence_transformer" in self.models,
    ...
    async def _process_task(
        ...
        try:
            ...
            document_id = str(document_id or f"doc_{hash(text)}")
            ...
            similar_analyses = await self._find_similar_precedent_analyses(
                text, document_id
            )
            logger.info(
                f"Found {len(similar_analyses)} similar precedent analyses in collective memory"
            )
            ...
            analysis_result = PrecedentAnalysisResult(
                ...
                confidence_score=self._calculate_confidence_score(
                    extracted_citations, precedent_matches
                ),
            )
            ...
            logger.info(
                f"Found {len(analysis_result.precedent_matches)} precedents in {processing_time:.2f}s"
            )
            ...
            return {
                ...
                "precedent_analysis": {
                    ...
                    "precedent_matches": [
                        match.__dict__ for match in analysis_result.precedent_matches
                    ],
            ...
            logger.error(
                f"Precedent analysis failed for {metadata.get('document_id', 'unknown')}: {e}"
            )
            raise

    async def _find_similar_precedent_analyses(
    ...
    async def _extract_citations(self, text: str) -> List[ExtractedCitation]:
        ...
        for category, patterns in self.citation_patterns.items():
            ...
            for pattern in patterns:
                citations = await self._extract_citations_by_pattern(
    ...
    async def _extract_citations_by_pattern(
        self, text: str, pattern: CitationPattern, category: str
        ...
        try:
            ...
            logger.warning(
                f"Failed to extract citations with pattern {pattern.pattern_type}: {e}"
            )

        return citations

    def _create_citation_from_match(
        self, match: re.Match, pattern: CitationPattern, category: str
    ...
    def _parse_case_citation(
        self, citation: ExtractedCitation, groups: Tuple
    ) -> ExtractedCitation:
    ...
    def _deduplicate_citations(
        ...
        for citation in sorted_citations:
            ...
            for i, existing in enumerate(deduplicated):
                if (
                    citation.start_pos < existing.end_pos
                    and citation.end_pos > existing.start_pos
                ):
                    overlapping = i
                    break
    ...
    async def _match_precedents(
        self, citations: List[ExtractedCitation], context_text: str
    ) -> List[PrecedentMatch]:
        ...
        for citation in citations:
            try:
                ...
                factual_sim = await self._calculate_factual_similarity(
                    citation, context_text
                )
                legal_sim = await self._calculate_legal_similarity(
                    citation, context_text
                )
                procedural_sim = await self._calculate_procedural_similarity(
                    citation, context_text
                )
                ...
                logger.warning(
                    f"Failed to match precedent for citation {citation.text}: {e}"
                )

        # Sort by combined score (similarity * authority weight)
        precedent_matches.sort(
            key=lambda m: m.similarity_score * m.authority_weight, reverse=True
        )

        # Limit results
        return precedent_matches[: self.config.max_precedents_returned]

    async def _calculate_factual_similarity(
    ...
    def _calculate_authority_weight(self, citation: ExtractedCitation) -> float:
        ...
        base_weight = self.authority_hierarchy.get(citation.authority_level, {}).get(
            "weight", 0.5
        )
    ...
    def _calculate_confidence_score(
        self, citations: List[ExtractedCitation], matches: List[PrecedentMatch]
    ) -> float:
    ...
    async def _enhance_with_collective_intelligence(  # noqa: C901
        self, analysis_result: PrecedentAnalysisResult, similar_analyses: List[Any]
        ...
        try:
                        ...
                        if "authority_distribution" in metadata:
                            ...
                            for match in analysis_result.precedent_matches:
                                if match.precedent_type in similar_authorities:
                                    ...
                                    match.collective_intelligence_score = (
                                        collective_boost
                                    )
            ...
            if collective_boost > 0:
                analysis_result.confidence_score = min(
                    1.0, analysis_result.confidence_score + collective_boost
                )
    ...
    async def _store_precedent_analysis(
        self, analysis_result: PrecedentAnalysisResult, document_id: str, text: str
    ):
        ...
        analysis_data = {
            "citations_extracted": [
                citation.__dict__ for citation in analysis_result.citations_extracted
            ],
            "precedent_matches": [
                match.__dict__ for match in analysis_result.precedent_matches
            ],
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        precedent_health = {
            ...
            "citation_patterns_loaded": sum(
                len(patterns) for patterns in self.citation_patterns.values()
            ),
...
async def create_legal_precedent_analyzer(
    ...
    logger.info(
        "Created Legal Precedent Analyzer Agent with collective intelligence capabilities"
    )
