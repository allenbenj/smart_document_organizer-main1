...
_llm_mod = try_import("core.llm_providers")
_model_switcher_mod = try_import("core.model_switcher")

LLMManager = getattr(_llm_mod, "LLMManager", None)
LLMProviderEnum = getattr(_llm_mod, "LLMProviderEnum", None)
TaskComplexity = getattr(_model_switcher_mod, "TaskComplexity", None)
    ...
    def update_from_feedback(self, feedback: List[Dict[str, Any]]):
        ...
        for fb in feedback:
            if fb.get("type") == "missing_components":
                self.prompt_tuning["completeness"] = (
                    "*Do not omit any Toulmin component*"
                )
            elif fb.get("type") == "incorrect_component_classification":
                self.prompt_tuning["precision"] = (
                    "*Be precise in assigning each sentence to its correct component*"
                )
...
class ToulminAnalyzer:
    ...
    def __init__(
        ...
        config: Optional[ToulminConfig] = None,
    ):
        self.llm_manager = llm_manager
        self.config = config or ToulminConfig()
    ...
    async def analyze_async(
        ...
        try:
            ...
            if self.config.use_llm_enhancement and self.llm_manager:
                enhanced_components = await self._enhance_with_llm(
                    document_content, pattern_components, context
                )
            ...
            return {
                ...
                "model_used": (
                    "pattern+llm" if self.config.use_llm_enhancement else "pattern"
                ),
            }
                        ...
                        components[component_type].append(
                            {
                                ...
                                "confidence": confidence,
                                "paragraph_index": i,
                                "type": self._classify_component_type(
                                    paragraph, component_type
                                ),
    ...
    async def _enhance_with_llm(
        self,
        document_content: str,
        pattern_components: Dict[str, List[Dict[str, Any]]],
        context: Dict[str, Any],
    ) -> Dict[str, List[Dict[str, Any]]]:
        try:
            trimmed_content = (
                document_content[:3500] + "..."
                if len(document_content) > 3500
            ...
            complexity = context.get(
                "complexity", TaskComplexity.MODERATE if TaskComplexity else "moderate"
            )
            ...
            self.logger.warning(
                f"LLM enhancement failed, using pattern results: {str(e)}"
            )
            return pattern_components
            ...
            for comp_type in component_types:
                ...
                if isinstance(components, list):
                    validated_components[comp_type] = [
                        ...
                        and comp.get("confidence", 0) >= self.config.min_confidence
                    ]
                else:
                    validated_components[comp_type] = []
    ...
    def _merge_components(
        self,
        pattern_components: Dict[str, List[Dict[str, Any]]],
        llm_components: Dict[str, List[Dict[str, Any]]],
    ) -> Dict[str, List[Dict[str, Any]]]:
        merged = {}
        for comp_type in self.toulmin_patterns.keys():
            ...
            for pc in pattern_comps:
                if not any(
                    self._text_similarity(pc["text"], lc["text"]) > 0.6
                    for lc in high_conf_llm
                ):
                    unique_pattern.append(pc)
            merged[comp_type] = (high_conf_llm + unique_pattern)[
    ...
    def _build_argument_structures(
        ...
        for i, claim in enumerate(claims):
            ...
            closest_data = self._find_closest_component(
                claim_index, components.get("data", [])
            )
            closest_warrant = self._find_closest_component(
                claim_index, components.get("warrants", [])
            )
            closest_backing = (
                self._find_closest_component(claim_index, components.get("backing", []))
                if closest_warrant
                else None
            )
            closest_rebuttal = self._find_closest_component(
                claim_index, components.get("rebuttals", [])
            )
            closest_qualifier = self._find_closest_component(
                claim_index, components.get("qualifiers", [])
            )
            ...
            structure = {
                ...
                "strength": self._assess_argument_strength(
                    claim, closest_data, closest_warrant, closest_backing
                ),
            }
    ...
    def _find_closest_component(
        self, reference_index: int, components: List[Dict[str, Any]]
    ...
    def _assess_completeness(
        ...
        essential_present = sum(
            1 for comp in ["claims", "data", "warrants"] if component_counts[comp] > 0
        )
        essential_completeness = essential_present / 3
        full_completeness = (
            sum(1 for count in component_counts.values() if count > 0) / 6
        )
        return {
    ...
    def _get_model_for_complexity(self, complexity: Any) -> Dict[str, Any]:
        # This codebase uses xAI provider via LLMManager.
        default_model = "grok-4-fast-reasoning"
        provider = getattr(LLMProviderEnum, "XAI", "xai") if LLMProviderEnum else "xai"
