...
from core.container import ProductionServiceContainer
...
class LegalSemanticAnalyzer(BaseAgent, MemoryEnabledMixin):
    """
    ...
    def __init__(
        ...
        self.document_type_indicators = {
            'brief': ['brief', 'memorandum', 'argument', 'statement of facts'],
            'motion': ['motion to', 'moves', 'respectfully moves', 'movant'],
            'complaint': ['complaint', 'plaintiff', 'comes now', 'cause of action'],
        ...
        self.legal_domain_indicators = {
            'litigation': ['lawsuit', 'court', 'trial', 'discovery', 'motion', 'brief'],
            'contracts': ['agreement', 'contract', 'breach', 'consideration', 'covenant'],
            'criminal': ['criminal', 'felony', 'misdemeanor', 'prosecution', 'defendant'],
    ...
    def _initialize_models(self):
        ...
        if TRANSFORMERS_AVAILABLE:
            try:
                ...
                logger.info(f"Loaded summarization model: {self.config.summarization_model}")
    ...
    def _initialize_legal_patterns(self):
        ...
        if not SPACY_AVAILABLE or 'spacy' not in self.models:
            return
        
        nlp = self.models['spacy']
        self.legal_patterns['matcher'] = Matcher(nlp.vocab)
        
        # Legal reasoning patterns
        legal_reasoning_patterns = [
            # IRAC patterns
            [{"LOWER": {"IN": ["issue", "question", "problem"]}}, {"IS_ALPHA": True, "OP": "*"}],
            [{"LOWER": {"IN": ["rule", "law", "statute", "regulation"]}}, {"IS_ALPHA": True, "OP": "*"}],
            [{"LOWER": {"IN": ["application", "analysis", "apply"]}}, {"IS_ALPHA": True, "OP": "*"}],
            [{"LOWER": {"IN": ["conclusion", "holding", "therefore"]}}, {"IS_ALPHA": True, "OP": "*"}],
            
            # Legal relationships
            [{"LOWER": "plaintiff"}, {"LOWER": "v."}, {"LOWER": "defendant"}],
            [{"LOWER": {"IN": ["breach", "violated", "infringed"]}}, {"IS_ALPHA": True, "OP": "*"}],
            [{"LOWER": {"IN": ["damages", "relief", "remedy"]}}, {"IS_ALPHA": True, "OP": "*"}],
        ]
        ...
        procedural_patterns = [
            ...
            [{"LOWER": {"IN": ["discovery", "deposition", "interrogatory"]}}, {"IS_ALPHA": True, "OP": "*"}],
        ]
        ...
        logger.info(f"Initialized {len(legal_reasoning_patterns + procedural_patterns)} legal patterns")
    
    def _log_available_models(self):
        """Log available models for semantic analysis"""
        
        models_status = {
            'Transformers (Summarization)': TRANSFORMERS_AVAILABLE and 'summarizer' in self.models,
            'Legal-BERT': TRANSFORMERS_AVAILABLE and 'legal_bert_model' in self.models,
            'spaCy': SPACY_AVAILABLE and 'spacy' in self.models,
            'scikit-learn (Topic Modeling)': SKLEARN_AVAILABLE and 'tfidf_vectorizer' in self.models,
            'NLTK': NLTK_AVAILABLE
        }
    ...
    async def _process_task(self, task_data: Any, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            similar_analyses = await self._find_similar_semantic_analyses(text, document_id)
            logger.info(f"Found {len(similar_analyses)} similar semantic analyses in collective memory")
            ...
            logger.info(f"Generated summary, {len(analysis_result.key_topics)} topics, {len(analysis_result.legal_concepts)} concepts in {processing_time:.2f}s")
            ...
            return {
                ...
                'semantic_analysis': {
                    ...
                    'legal_concepts': [concept.__dict__ for concept in analysis_result.legal_concepts],
            ...
            logger.error(f"Semantic analysis failed for {metadata.get('document_id', 'unknown')}: {e}")
            raise
    
    async def _find_similar_semantic_analyses(self, text: str, document_id: str) -> List[Any]:
    ...
    def _extract_semantic_keywords(self, text: str) -> List[str]:
        ...
        legal_terms = [
            'court', 'law', 'legal', 'statute', 'regulation', 'contract',
            'plaintiff', 'defendant', 'liability', 'damages', 'breach',
            'jurisdiction', 'motion', 'brief', 'judgment', 'precedent'
        ]
    ...
    async def _analyze_document_semantics(self, text: str, document_id: str) -> SemanticAnalysisResult:
    ...
    async def _generate_summary(self, text: str) -> str:
        ...
        try:
            ...
            logger.warning(f"Transformer summarization failed, using extractive method: {e}")
            return self._extractive_summary(text)
    ...
    def _extractive_summary(self, text: str) -> str:
        ...
        legal_keywords = [
            'court', 'plaintiff', 'defendant', 'law', 'statute', 'contract',
            'liability', 'damages', 'jurisdiction', 'motion', 'brief'
        ]
    ...
    async def _identify_legal_topics(self, text: str) -> List[LegalTopic]:
        ...
        ranked_topics = sorted(unique_topics, key=lambda t: t.relevance_score, reverse=True)
    ...
    def _deduplicate_topics(self, topics: List[LegalTopic]) -> List[LegalTopic]:
        ...
        for topic in topics:
            ...
            for existing in unique_topics:
                ...
                if keyword_overlap > 0 or topic.topic_category == existing.topic_category:
                    # Merge topics
    ...
    async def _extract_legal_concepts(self, text: str) -> List[SemanticConcept]:
        ...
        filtered_concepts = [c for c in concepts if c.confidence >= self.config.min_concept_confidence]
        ranked_concepts = sorted(filtered_concepts, key=lambda c: c.confidence, reverse=True)
        
        return ranked_concepts[:self.config.max_concepts_per_category * 4]  # 4 categories
        ...
        for doc_type, indicators in self.document_type_indicators.items():
            ...
            if matches > 0:
                ...
                if confidence > doc_type_confidence:
                    ...
                    doc_features = [indicator for indicator in indicators if indicator in text_lower]
    ...
    async def _extract_semantic_relationships(self, text: str) -> List[Dict[str, Any]]:
        ...
        for rel in relationships:
            if any(term in rel['context'].lower() for term in ['court', 'law', 'legal', 'contract', 'statute']):
    ...
    def _calculate_semantic_confidence(
        ...
        # Summary confidence
        if summary:
            summary_confidence = min(1.0, len(summary.split()) / 50.0)  # Based on summary length
        ...
        if relationships:
            rel_confidence = sum(r['confidence'] for r in relationships) / len(relationships)
    ...
    async def _enhance_with_collective_intelligence(
        ...
        try:
                ...
                try:
                    if hasattr(similar.record, 'metadata') and similar.record.metadata:
                        ...
                        if 'document_type' in metadata:
                            if metadata['document_type'] == analysis_result.document_classification.document_type:
                                enhancement_boost += 0.1
                        
                        # Boost confidence for similar legal domains
                        if 'legal_domain' in metadata:
                            if metadata['legal_domain'] == analysis_result.document_classification.legal_domain:
                                enhancement_boost += 0.1
            ...
            if enhancement_boost > 0:
                analysis_result.confidence_score = min(1.0, analysis_result.confidence_score + enhancement_boost)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        semantic_health = {
            ...
            'legal_patterns_loaded': len(self.document_type_indicators) + len(self.legal_domain_indicators)
        }
...
async def create_legal_semantic_analyzer(
    ...
    logger.info(f"Created Legal Semantic Analyzer Agent with collective intelligence capabilities")
