...
from mem_db.memory.memory_interfaces import MemoryType
...
class IracAnalyzerAgent(BaseAgent, LegalDomainMixin, MemoryEnabledMixin):
    """
    ...
    def __init__(
        ...
        config: Optional[IracConfig] = None
    ):
    ...
    def _compile_patterns(self):
        ...
        self._pattern_cache['issue'] = [
            re.compile(r'(?i)(?:the\s+)?(?:issue|question|problem)\s+(?:is|presented|here)\s+(.+?)(?:\.|;|\n\n)', re.DOTALL),
            re.compile(r'(?i)(?:issue|question):\s*(.+?)(?:\n\n|\n(?=[A-Z])|$)', re.DOTALL),
            re.compile(r'(?i)whether\s+(.+?)(?:\.|;|\n)', re.DOTALL),
        ]
        
        # Rule patterns
        self._pattern_cache['rule'] = [
            re.compile(r'(?i)(?:the\s+)?(?:rule|law|statute|regulation|principle)\s+(?:states|provides|holds)\s+(.+?)(?:\.|;|\n\n)', re.DOTALL),
        ...
        self._pattern_cache['application'] = [
            re.compile(r'(?i)(?:applying|application|analysis|here)\s+(.+?)(?:\n\n|\n(?=[A-Z])|$)', re.DOTALL),
        ...
        self._pattern_cache['conclusion'] = [
            re.compile(r'(?i)(?:therefore|thus|consequently|in\s+conclusion)\s+(.+?)(?:\.|;|\n\n)', re.DOTALL),
            re.compile(r'(?i)conclusion:\s*(.+?)(?:\n\n|\n(?=[A-Z])|$)', re.DOTALL),
            re.compile(r'(?i)(?:the\s+)?(?:court|result|outcome)\s+(?:held|finds|concludes)\s+(.+?)(?:\.|;|\n)', re.DOTALL),
        ]
    ...
    async def _process_task(self, task_data: Any, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            similar_analyses = await self._find_similar_irac_analyses(document_text)
            logger.info(f"Found {len(similar_analyses)} similar IRAC analyses from collective memory")
            
            # Step 2: Perform pattern-based IRAC extraction
            pattern_result = await self._extract_irac_patterns(document_text)
            
            # Step 3: Enhance with LLM analysis if configured
            if self.config.use_llm_validation:
                llm_result = await self._llm_irac_analysis(document_text, similar_analyses)
                # Combine pattern and LLM results
                irac_analysis = await self._combine_irac_results(pattern_result, llm_result, document_id)
            else:
                irac_analysis = await self._create_irac_from_patterns(pattern_result, document_id)
            ...
            if self.config.enable_nested_irac:
                nested_analyses = await self._detect_nested_irac(document_text, irac_analysis)
            ...
            logger.info(f"IRAC analysis completed for document {document_id} with confidence {irac_analysis.overall_confidence:.2f}")
            ...
            return {
                ...
                'metadata': {
                    ...
                    'analysis_method': 'hybrid_pattern_llm' if self.config.use_llm_validation else 'pattern_based',
            ...
            logger.error(f"IRAC analysis failed for document {metadata.get('document_id', 'unknown')}: {e}")
            raise
        ...
        for component_type in ['issue', 'rule', 'application', 'conclusion']:
            ...
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    if len(content) > 10:  # Minimum content length
                        ...
                        confidence = self._calculate_pattern_confidence(content, component_type)
            ...
            if best_match:
                ...
                components[component_type] = IracComponent(
                    ...
                    confidence=0.1,
                    metadata={'extraction_method': 'pattern_matching', 'status': 'not_found'}
                )
    ...
    def _calculate_pattern_confidence(self, content: str, component_type: str) -> float:
        ...
        keyword_factor = sum(1 for keyword in keywords if keyword.lower() in content.lower()) / len(keywords)
        
        # Combine factors
        confidence = base_confidence + (length_factor * 0.3) + (keyword_factor * 0.2)
        return min(1.0, confidence)
    
    async def _llm_irac_analysis(self, text: str, similar_analyses: List[Any]) -> Dict[str, Any]:
        ...
        if similar_analyses:
            ...
            for i, result in enumerate(similar_analyses[:3], 1):
                try:
                    ...
                    collective_context += f"\n{i}. Issue: {analysis_data.get('issue', {}).get('content', 'N/A')[:100]}..."
    ...
    async def _combine_irac_results(
        ...
        overall_confidence = sum(comp.confidence for comp in combined_components.values()) / 4
    ...
    async def _create_irac_from_patterns(
        ...
        overall_confidence = sum(comp.confidence for comp in pattern_result.values()) / 4
    ...
    async def _find_relevant_precedents(self, irac_analysis: IracAnalysis) -> List[Dict[str, Any]]:
    ...
    async def _detect_nested_irac(self, text: str, main_analysis: IracAnalysis) -> List[IracAnalysis]:
        ...
        for i, section in enumerate(sections):
            if len(section.split()) > 50:  # Minimum section size
                ...
                section_confidence = sum(comp.confidence for comp in section_analysis.values()) / 4
                
                if section_confidence > 0.3:  # Threshold for valid nested IRAC
                    nested_irac = await self._create_irac_from_patterns(
                        section_analysis, 
                        f"{main_analysis.document_id}_section_{i}"
                    )
...
async def create_irac_analyzer(
    ...
    logger.info(f"Created IRAC Analyzer Agent with collective intelligence capabilities")
