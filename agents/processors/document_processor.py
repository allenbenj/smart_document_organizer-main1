...
from agents.base.agent_mixins import DocumentProcessingMixin, LegalMemoryMixin  # noqa: E402

if TYPE_CHECKING:
    from mem_db.memory.memory_interfaces import MemoryType
else:
    try:
        from mem_db.memory.memory_interfaces import MemoryType  # type: ignore  # noqa: E402
...
try:
    import pytesseract  # type: ignore[reportMissingImports,import-not-found]  # noqa: E402
...
try:
    import whisper  # type: ignore[reportMissingImports,import-not-found]  # noqa: E402
    from moviepy.editor import (  # type: ignore[reportMissingImports,import-not-found]  # noqa: E402
        VideoFileClip,
    )
...
class ProcessedDocument:
    ...
    multimodal_components: Dict[str, Any] = field(
...
class DocumentProcessor(BaseAgent, DocumentProcessingMixin, LegalMemoryMixin):
    """
    ...
    def _log_available_dependencies(self):
        ...
        logger.info(
            f"Available dependencies: {', '.join(available) if available else 'None'}"
        )
        if unavailable:
            logger.info(
    ...
    async def _process_task(  # noqa: C901
        ...
        try:
            ...
            if isinstance(task_data, str):
                ...
                if p.exists() and p.is_file():
                    ...
                    document_id = metadata.get(
                        "document_id",
                        f"doc_{file_path.stem}_{file_path.stat().st_size}",
                    )
                else:
                    content = task_data
                    content_hash = hashlib.sha256(content.encode("utf-8")).hexdigest()
                    document_id = metadata.get(
                        "document_id", f"content_{content_hash[:16]}"
                    )
            elif isinstance(task_data, dict):
                file_path = (
                    Path(task_data["file_path"]) if "file_path" in task_data else None
                )
            ...
            if not file_path and not content:
                raise ValueError(
                    "No file path or content provided for document processing."
                )
            ...
            if self._is_memory_available():
                ...
                if similar_docs:
                    logger.info(
                        f"Found {len(similar_docs)} similar documents. Enhancing..."
                    )
                    processed_doc = self._enhance_with_collective_intelligence(
                        processed_doc, similar_docs
                    )
            ...
            logger.info(
                f"Document processing completed for {document_id} in {processing_time:.2f}s"
            )
    ...
    async def _process_file(
        ...
        logger.info(
            f"Processing file: {file_path} (ext: {file_extension}, size: {file_size} bytes)"
        )
    ...
    async def _process_docx(self, file_path: Path) -> tuple:
        ...
        for table in doc.tables:
            for row in table.rows:
                full_text.append(" | ".join(cell.text for cell in row.cells))  # noqa: F821
        return "\n".join(full_text), metadata, None, None, "python-docx"  # noqa: F821
    ...
    async def _process_image(self, file_path: Path) -> tuple:
        if not OCR_AVAILABLE:
            raise RuntimeError(
                "pytesseract/Pillow not installed, cannot process images."
            )

        if Image is None or pytesseract is None:
            raise RuntimeError("OCR libraries are unavailable")
    ...
    def _determine_processing_strategy(self, file_extension: str) -> str:
        ...
        elif file_extension in [".png", ".jpg", ".jpeg", ".tif", ".bmp"]:
    ...
    def _get_document_format(self, file_extension: str) -> str:
        ...
        return getattr(
            DocumentFormat, file_extension.lstrip(".").upper(), DocumentFormat.UNKNOWN
        )

    def _calculate_processing_confidence(
        self, method: str, content: str, ext: str
    ) -> float:
    ...
    def _create_chunks(self, text: str) -> List[Dict[str, Any]]:
        ...
        while start < len(text):
            ...
            chunktext = text[start:end]  # noqa: F841
            chunks.append(
                {
    ...
    def _detect_legal_content(self, content: str, filename: str) -> List[str]:
        ...
        if not self.config.legal_document_detection:
            return []
    ...
    def _update_statistics(self, doc: ProcessedDocument):
        ...
        self.stats["formats_processed"][doc.document_format] = (
            self.stats["formats_processed"].get(doc.document_format, 0) + 1
        )

        total_time = self.stats["total_processing_time"] + doc.processing_time
        self.stats["total_processing_time"] = total_time
        self.stats["avg_processing_time"] = (
            total_time / self.stats["documents_processed"]
        )
    ...
    async def _find_similar_documents(self, doc: ProcessedDocument) -> List[Any]:
        ...
        try:
            return await self.search_memory(
                query=query,
                memory_types=[cast(Any, MemoryType).DOCUMENT],
                limit=3,
                min_similarity=0.7,
            )
        except Exception as e:
    ...
    def _enhance_with_collective_intelligence(  # noqa: C901
        self, doc: ProcessedDocument, similar_docs: List[Any]
    ) -> ProcessedDocument:
    ...
    async def _store_processed_document(self, doc: ProcessedDocument):
        ...
        try:
            await self.store_memory(
                ...
                memory_type=cast(Any, MemoryType).DOCUMENT,
