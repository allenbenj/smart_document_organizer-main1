...
async def analyze_legal(
    ...
    try:
        ...
        result = await service.dispatch_task(
            "analyze_legal", {"text": payload.text, "context": opts}
        )
                ...
                await svc.create_proposal(
                    {
                        ...
                        "confidence_score": float(
                            (result.metadata or {}).get("confidence", 0.5)
                        ),
        ...
        if not result.success:
            deg = (
                (result.metadata or {}).get("degradation") if result.metadata else None
            )
...
async def submit_feedback(
    payload: FeedbackPayload, service: AgentService = Depends(get_agent_service)
) -> Dict[str, Any]:
    try:
        stored = await service.dispatch_task(
            "submit_feedback",
            {
...
async def analyze_irac(
    payload: TextPayload, service: AgentService = Depends(get_agent_service)
) -> Dict[str, Any]:
    try:
        result = await service.dispatch_task(
            "analyze_irac", {"text": payload.text, "options": payload.options}
        )

        if isinstance(result, dict) and not result.get("success", True):
            raise HTTPException(
                status_code=503, detail=result.get("error", "unavailable")
            )
...
async def analyze_toulmin(
    ...
    try:
        result = await service.dispatch_task(
            "analyze_toulmin", {"text": payload.text, "options": payload.options}
        )
...
async def semantic_analysis(
    ...
    try:
        result = await service.dispatch_task(
            "analyze_semantic", {"text": payload.text, "options": payload.options}
        )
...
async def contradictions(
    ...
    try:
        result = await service.dispatch_task(
            "analyze_contradictions", {"text": payload.text, "options": payload.options}
        )

        if isinstance(result, dict) and not result.get("success", True):
            raise HTTPException(
                status_code=503, detail={"error": result.get("error", "unavailable")}
            )
...
async def violations(
    ...
    try:
        result = await service.dispatch_task(
            "analyze_violations", {"text": payload.text, "options": payload.options}
        )
...
async def analyze_contract(
    ...
    try:
        result = await service.dispatch_task(
            "analyze_contract", {"text": payload.text, "options": payload.options}
        )
...
async def check_compliance(
    ...
    try:
        result = await service.dispatch_task(
            "check_compliance", {"text": payload.text, "options": payload.options}
        )
...
async def embed(
    payload: EmbedPayload, service: AgentService = Depends(get_agent_service)
) -> Dict[str, Any]:
    try:
        result = await service.dispatch_task(
            "embed_texts", {"texts": payload.texts, "options": payload.options}
        )
        if isinstance(result, dict) and not result.get("success", True):
            raise HTTPException(
                status_code=500, detail=result.get("error", "Embedding failed")
            )
...
async def orchestrate(
    ...
    try:
        result = await service.dispatch_task(
            "orchestrate_task", {"text": payload.text, "options": payload.options}
        )
        if isinstance(result, dict) and not result.get("success", True):
            raise HTTPException(
                status_code=500, detail=result.get("error", "Orchestration failed")
            )
...
async def classify(
    payload: ClassifyPayload, service: AgentService = Depends(get_agent_service)
) -> Dict[str, Any]:
    try:
        result = await service.dispatch_task(
            "classify_text", {"text": payload.text, "options": payload.options}
        )
        if isinstance(result, dict) and not result.get("success", True):
            raise HTTPException(
...
async def process_document(
    ...
    try:
        if file_id is not None:
            ...
            if not rec:
                raise HTTPException(
                    status_code=404, detail=f"Indexed file not found: {file_id}"
                )
            if rec.get("status") != "ready":
                raise HTTPException(
                    status_code=400,
                    detail={
                        "error": "indexed_file_not_ready",
            ...
            if file is None:
                raise HTTPException(
                    status_code=400, detail="Provide either file upload or file_id"
                )
...
async def process_documents(
    ...
    for i, up in enumerate(files):
        logger.info("Processing file %d: %s", i + 1, up.filename)
        suffix = os.path.splitext(up.filename or "")[1] or ""
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            blob = await up.read()
            tmp.write(blob)
            tmp_path = tmp.name
        logger.info("Created temp file: %s", tmp_path)
        ...
        try:
            ...
            if result is None:
                logger.error("Processing failed: empty result for %s", up.filename)
                result = {
                    "success": False,
                    "error": "Processing failed: empty result",
                    "data": {},
            ...
            logger.info(
                "File %s processed successfully: %s",
                up.filename,
                validated.get("success"),
            )
        except Exception as e:
            logger.error("Exception processing file %s: %s", up.filename, e)
            raise
        finally:
            try:
                os.unlink(tmp_path)
