...
class ConsoleLogHandler(LogHandler):
    ...
    async def emit(self, record: LogRecord) -> None:
        ...
        try:
            ...
            if self.use_colors and hasattr(self.stream, 'isatty') and self.stream.isatty():
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=self.timeout)) as session:
                async with session.post(self.endpoint, json=payload, headers=headers) as response:
                    if response.status >= 400:
                        raise LogHandlerError(f"Remote logging failed with status {response.status}")
    ...
    async def _background_flush(self) -> None:
        ...
        while True:
            try:
                ...
                async with self._lock:
                    if self._batch and (datetime.now() - self._last_flush).total_seconds() >= self.flush_interval:
                        await self._flush_batch()
