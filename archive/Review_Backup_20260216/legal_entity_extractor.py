...
from ...agents.base.agent_mixins import LegalDomainMixin, MemoryEnabledMixin
...
class LegalEntityExtractor(BaseAgent, LegalDomainMixin, MemoryEnabledMixin):
    """
    ...
    def _initialize_models(self):
        ...
        if self.config.use_spacy and SPACY_AVAILABLE:
            try:
                ...
                logger.warning(f"Could not load spaCy model {self.config.spacy_model}, trying smaller model")
                try:
                    self.spacy_nlp = spacy.load("en_core_web_sm")
                    logger.info("Loaded fallback spaCy model: en_core_web_sm")
                except OSError:
                    logger.warning("No spaCy model available, disabling spaCy extraction")
                    self.config.use_spacy = False
        
        # Initialize GLiNER if available and configured
        if self.config.use_gliner and GLINER_AVAILABLE:
            try:
                self.gliner_model = GLiNER.from_pretrained(self.config.gliner_model)
    ...
    def _compile_legal_patterns(self) -> Dict[str, List[re.Pattern]]:
        ...
        patterns = {
            ...
            'Statute': [
                ...
                re.compile(r'\bSection\s+\d+\s+of\s+the\s+[A-Za-z\s]+Act\b'),  # Act sections
                re.compile(r'\b[A-Za-z\s]+Act\s+of\s+\d{4}\b'),  # Named acts
            ],
            
            # Legal rules and procedures
            'LegalRule': [
                re.compile(r'\bRule\s+\d+(?:\(\w+\))?(?:\(\d+\))?\s+of\s+the\s+Federal\s+Rules\s+of\s+\w+\b'),
                re.compile(r'\bFed\.?\s*R\.?\s*\w+\.?\s*\d+\b'),  # Federal Rules abbreviations
            ],
    ...
    async def _process_task(self, task_data: Any, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(task_data, str):
                ...
                document_id = task_data.get('document_id', metadata.get('document_id', f"doc_{id(task_data)}"))
            else:
            ...
            logger.info(f"Found {len(similar_extractions)} similar entity extractions from collective memory")
            ...
            if self.config.use_spacy and self.spacy_nlp:
                spacy_entities = await self._extract_with_spacy(document_text)
            ...
            if self.config.use_gliner and self.gliner_model:
                gliner_entities = await self._extract_with_gliner(document_text)
            ...
            if self.config.use_patterns:
                pattern_entities = await self._extract_with_patterns(document_text)
                all_entities.extend(pattern_entities)
                extraction_methods_used.add('patterns')
                logger.info(f"Pattern matching extracted {len(pattern_entities)} entities")
            
            # Method 4: LLM enhancement
            if self.config.use_llm_enhancement:
                llm_entities = await self._extract_with_llm(document_text, all_entities, similar_extractions)
                all_entities.extend(llm_entities)
                extraction_methods_used.add('llm')
                logger.info(f"LLM enhancement extracted {len(llm_entities)} additional entities")
            
            # Step 3: Deduplication and entity resolution
            if self.config.enable_deduplication:
                unique_entities = await self._deduplicate_entities(all_entities)
                logger.info(f"Deduplication: {len(all_entities)} → {len(unique_entities)} entities")
            ...
            if self.config.enable_validation:
                ...
                logger.info(f"Validation: {len(all_entities)} → {len(validated_entities)} valid entities")
                all_entities = validated_entities
            
            # Step 5: Extract relationships between entities
            relationships = await self._extract_relationships(all_entities, document_text)
            
            # Step 6: Calculate overall confidence and statistics
            processing_time = (datetime.now() - start_time).total_seconds()
            overall_confidence = sum(entity.confidence for entity in all_entities) / len(all_entities) if all_entities else 0.0
            ...
            logger.info(f"Extracted {len(all_entities)} entities in {processing_time:.2f}s with confidence {overall_confidence:.2f}")
            ...
            logger.error(f"Entity extraction failed for document {metadata.get('document_id', 'unknown')}: {e}")
            raise
    ...
    async def _find_similar_extractions(self, document_text: str) -> List[Any]:
        ...
        try:
            ...
            for result in similar_results:
                ...
                if 'entity' in metadata.get('analysis_type', '') or result.record.memory_type == MemoryType.ENTITY:
    ...
    def _extract_key_terms(self, text: str) -> List[str]:
        ...
        legal_keywords = [
            'court', 'judge', 'plaintiff', 'defendant', 'contract', 'agreement',
            'statute', 'regulation', 'case', 'law', 'legal', 'constitutional'
        ]
    ...
    async def _extract_with_gliner(self, text: str) -> List[ExtractedEntity]:
        ...
        try:
            ...
            legal_labels = [
                "Person", "Organization", "Court", "Case", "Statute", 
                "Legal Concept", "Contract", "Location", "Date", "Money"
            ]
            ...
            for ent in gliner_entities:
                ...
                entity = ExtractedEntity(
                    ...
                    start_pos=ent['start'],
                    end_pos=ent['end'],
                    confidence=ent.get('score', 0.8),  # GLiNER provides confidence scores
            ...
            for pattern in patterns:
                for match in pattern.finditer(text):
                    ...
                    entity = ExtractedEntity(
                        ...
                        attributes={
                            'pattern_type': 'regex',
                            'matched_pattern': pattern.pattern[:50] + '...' if len(pattern.pattern) > 50 else pattern.pattern
                        }
                    )
    ...
    async def _extract_with_llm(
        ...
        try:
            ...
            llm_service = None
            try:
                llm_service = self.services.get_service("llm_manager")
            except:
                pass
            ...
            if similar_extractions:
                ...
                for i, result in enumerate(similar_extractions[:3], 1):
                    try:
                        entity_data = json.loads(result.record.content) if isinstance(result.record.content, str) else result.record.content
    ...
    async def _deduplicate_entities(self, entities: List[ExtractedEntity]) -> List[ExtractedEntity]:
        ...
        while i < len(sorted_entities):
            ...
            j = i + 1
            while j < len(sorted_entities) and sorted_entities[j].start_pos < current_entity.end_pos:
    ...
    def _resolve_overlapping_entities(self, overlapping: List[ExtractedEntity]) -> List[ExtractedEntity]:
        ...
        overlapping.sort(key=lambda x: (x.confidence, method_priority.get(x.extraction_method, 0)), reverse=True)
    ...
    def _deduplicate_by_text(self, entities: List[ExtractedEntity]) -> List[ExtractedEntity]:
        ...
        for entity in entities:
            ...
            for seen_text in seen_texts:
                if self._text_similarity(normalized_text, seen_text) > 0.9:
                    is_duplicate = True
                    break
    ...
    async def _validate_entities(self, entities: List[ExtractedEntity]) -> List[ExtractedEntity]:
        ...
        for entity in entities:
            if self._is_valid_entity(entity):
                ...
                logger.debug(f"Invalid entity filtered out: {entity.entity_type} - {entity.text}")
    ...
    def _infer_relationship_type(self, type1: str, type2: str) -> Optional[str]:
        ...
        return relationship_rules.get((type1, type2)) or relationship_rules.get((type2, type1))
    ...
    async def _store_extraction_result(self, result: ExtractionResult) -> str:
        ...
        return await self.store_analysis_result(
            ...
            metadata={
                ...
                'extraction_methods': result.extraction_stats['extraction_methods_used']
            }
        )
    ...
    def _update_statistics(self, result: ExtractionResult, methods_used: Set[str]):
        ...
        self.stats['unique_entity_types'].update(result.extraction_stats['entity_types'])
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        extraction_health = {
            'extraction_config': {
                ...
                'gliner_enabled': self.config.use_gliner and self.gliner_model is not None,
            ...
            'pattern_count': sum(len(patterns) for patterns in self.legal_patterns.values())
        }
...
async def create_legal_entity_extractor(
    ...
    logger.info(f"Created Legal Entity Extractor Agent with collective intelligence capabilities")
