...
async def knowledge_status() -> Dict[str, Any]:
    try:
        ...
        return {"available": True, "initialized": bool(getattr(mgr, "_networkx_graph", None)), "stats": stats}
        ...
        if mgr is None:
            raise HTTPException(status_code=501, detail={
                ...
                "degradation": {
                    ...
                    "fits_workflow": False,
                    "suggested_actions": ["Install networkx", "Initialize Knowledge Manager"],
                },
            })
...
async def add_entity(payload: EntityPayload) -> Dict[str, Any]:
    try:
        mgr = get_knowledge_manager()
        if mgr is None:
            raise HTTPException(status_code=501, detail={
                ...
                "degradation": {
                    ...
                    "fits_workflow": False,
                    "suggested_actions": ["Install KG deps", "Initialize Knowledge Manager"],
                },
            })
        ...
        if payload.attributes:
            try:
                data.update({k: v for k, v in payload.attributes.items() if k not in data})
            except Exception:
                pass
        try:
            from mem_db.knowledge.unified_knowledge_graph_manager import LegalEntity
        ...
        if hasattr(mgr, "_networkx_graph") and mgr._networkx_graph is not None:
            mgr._networkx_graph.add_node(ent_id, **(entity.to_dict() if hasattr(entity, "to_dict") else data))
        # Update stats
...
class EntitiesImportPayload(BaseModel):
    items: List[Dict[str, Any]]  # each: { name: str, entity_type_label?: str, entity_type?: str, attributes?: dict }
...
async def add_relationship(payload: RelationshipPayload) -> Dict[str, Any]:
    try:
        mgr = get_knowledge_manager()
        if mgr is None:
            raise HTTPException(status_code=501, detail="Knowledge manager dependencies not available")
        ...
        if hasattr(mgr, "_networkx_graph") and mgr._networkx_graph is not None:
            mgr._networkx_graph.add_edge(payload.source_id, payload.target_id, key=rel_id, relation_type=payload.relation_type, **(payload.properties or {}))
        # Update stats
        if hasattr(mgr, "_stats"):
...
class TriplesPayload(BaseModel):
    ...
    entity_type_label: Optional[str] = None  # Ontology label (e.g., "Person", "Statute")
    create_missing: Optional[bool] = True
    use_heuristics: Optional[bool] = True


@router.post("/knowledge/import_triples")
async def import_triples(payload: TriplesPayload) -> Dict[str, Any]:
    ...
    try:
        ...
        if payload.entity_type_label:
            try:
                from agents.entities.ontology.ontology import get_entity_type_by_label
        ...
        def infer_entity_type_label(name: str) -> Optional[str]:
            try:
                ...
                # Court / Case
                if " court" in low or "court of" in low or "appeals" in low:
                    return "Court"
                ...
                if " regulation" in low or " cfr" in low:
                    return "Regulation"
                # Law Enforcement
                if "police" in low or "sheriff" in low or "fbi" in low:
        ...
        for head, rel, tail in payload.triples:
            ...
            if payload.create_missing:
                if not h_id:
                    inferred = infer_entity_type_label(head) if payload.use_heuristics else None
                    etype = default_type
                    if inferred:
                        try:
                            from agents.entities.ontology.ontology import get_entity_type_by_label
                    ...
                    res = await add_entity(EntityPayload(name=head, entity_type=etype, attributes=attrs))
                    h_id = res.get("id")
                    created_entities += 1
                if not t_id:
                    inferred = infer_entity_type_label(tail) if payload.use_heuristics else None
                    etype = default_type
                    if inferred:
                        try:
                            from agents.entities.ontology.ontology import get_entity_type_by_label
                    ...
                    res = await add_entity(EntityPayload(name=tail, entity_type=etype, attributes=attrs))
                    t_id = res.get("id")
                    created_entities += 1
            ...
            rel_payload = RelationshipPayload(source_id=h_id, target_id=t_id, relation_type=rel_norm, properties={})
            res = await add_relationship(rel_payload)
            if res.get("id"):
                created_rels += 1

        return {"created_entities": created_entities, "created_relationships": created_rels}
    except HTTPException:
        raise
    except Exception as e:
...
async def import_entities(payload: EntitiesImportPayload) -> Dict[str, Any]:
    ...
    try:
        ...
        for it in payload.items:
            ...
            res = await add_entity(EntityPayload(name=name, entity_type=etype, attributes=attrs))
            if res.get("id"):
                created += 1
