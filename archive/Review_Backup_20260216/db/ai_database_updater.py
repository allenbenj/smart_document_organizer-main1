...
class AIDatabaseUpdater:
    ...
    """
    def __init__(self, db_config: DatabaseConfig, ai_client: AIClient, ai_analyzer: AIAnalyzer):
        """
    ...
    def get_unanalyzed_files(self, limit: int = 50) -> List[Tuple[int, str, str]]:
        ...
        if not conn:
            logging.error("Failed to connect to file_tracker database to get unanalyzed files.")
            return []
    ...
    def save_analysis(self, file_path: str, analysis: Dict[str, Any]):
        ...
        if not conn:
            logging.error("Failed to connect to file_tracker database to save analysis.")
            return
    ...
    def update_file_status(self, file_id: int, status: str = 'analyzed'):
        ...
        conn = self.db_config.get_connection('file_tracker', read_only=False)
        if not conn:
            logging.error(f"Failed to connect to file_tracker to update status for file_id {file_id}.")
            return
    ...
    def run_analysis_batch(self, batch_size: int = 20, model_preference: str = "ollama") -> int:
        """
        ...
        for file_id, file_path, content in files:
            ...
            try:
                ...
                analysis = self.ai_analyzer.analyze_file_content(file_path, content, model_preference)

                if analysis:
                    self.save_analysis(file_path, analysis)
                    self.update_file_status(file_id, 'analyzed')
                    analyzed_count += 1
                    print(f" Successfully analyzed: {os.path.basename(file_path)} (Purpose: {analysis.get('primary_purpose', 'N/A')})")
                else:
                    self.update_file_status(file_id, 'analysis_failed')
                    print(f" AI analysis failed for {os.path.basename(file_path)}. See logs for details.")
    ...
    def generate_summary_report(self, model_preference: str = "deepseek") -> Optional[str]:
        """
        ...
        file_type_breakdown = chr(10).join([
...
"""
        ...
        if model_preference == "deepseek":
            ...
            report_content = self.ai_client.call_ollama(prompt, model="phi3:mini", max_tokens=2000)

        if not report_content:
            print(f"Failed to get response from {model_preference.capitalize()} for summary report.")
            return None
        ...
        try:
            with open(report_file_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"*Generated by {model_preference.capitalize()} AI on {timestamp}*\n\n")
...
if __name__ == "__main__":
    ...
    # Run analysis batch
    analyzed_count = updater.run_analysis_batch(batch_size=10, model_preference="ollama")
