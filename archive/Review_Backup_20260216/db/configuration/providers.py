...
class FileConfigurationProvider(ConfigurationProvider):
    ...
    async def load(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.format_type == ConfigurationFormat.JSON:
                ...
                raise ConfigurationSourceError(f"Unsupported format: {self.format_type}")
                
        except Exception as e:
            raise ConfigurationSourceError(f"Failed to load config from {self.file_path}: {e}")
    ...
    async def save(self, config: Dict[str, Any]) -> None:
        ...
        try:
            if self.format_type == ConfigurationFormat.JSON:
                ...
                raise ConfigurationSourceError(f"Saving not supported for format: {self.format_type}")
            
            self.file_path.write_text(content, encoding='utf-8')
            
        except Exception as e:
            raise ConfigurationSourceError(f"Failed to save config to {self.file_path}: {e}")
    ...
    def _load_toml(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigurationSourceError("tomli not installed for TOML support")
    ...
    async def load(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.connection_string.startswith('sqlite:'):
                ...
                raise ConfigurationSourceError(f"Unsupported database type: {self.connection_string}")
