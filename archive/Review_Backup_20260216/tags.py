...
from utils.models import TagCreate, TagResponse, DocumentTagCreate, DocumentResponse
...
async def list_tags():
    ...
    try:
        ...
        return [TagResponse(id=i, document_id=0, tag_name=t, created_at="2025-01-01T00:00:00Z") for i, t in enumerate(tags)]
...
@router.post("/documents/{document_id}/tags", status_code=status.HTTP_201_CREATED, response_model=List[TagResponse])
async def add_tag_to_document(document_id: int, tags: List[TagCreate]):
...
@router.delete("/documents/{document_id}/tags/{tag_name}", status_code=status.HTTP_200_OK)
async def remove_tag_from_document(document_id: int, tag_name: str):
    ...
    try:
        ...
        return {"message": f"Tag {tag_name} removed from document {document_id} successfully"}
...
async def get_documents_by_tag(tag_name: str, limit: int = 20, offset: int = 0):
