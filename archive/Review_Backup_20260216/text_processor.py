...
class TextProcessor:
    ...
    def __init__(
        ...
        if text_model is not None:
            ...
            # Resilient config resolution
            if config is None:
                from file_organizer.models.text_model import OLLAMA_AVAILABLE
                if OLLAMA_AVAILABLE:
                    config = TextModel.get_default_config()
                else:
                    # Fallback to DeepSeek if Ollama isn't here
                    from file_organizer.models import OpenAIModel
                    config = OpenAIModel.get_default_config()

            provider = config.framework or config.extra_params.get("provider", "ollama")
            
            if provider in ["openai", "deepseek", "xai", "deepseek"]:
                from file_organizer.models import OpenAIModel
                self.text_model = OpenAIModel(config)
            else:
        ...
        # Proactively initialize if possible (won't crash if cloud/properly configured)
        try:
            self.initialize()
        except Exception as e:
            logger.warning(f"Initial model initialization skipped/failed: {e}")
