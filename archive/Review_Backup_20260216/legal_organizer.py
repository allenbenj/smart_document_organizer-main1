...
class Config:
    def __init__(self, config_path="config.yaml"):
        ...
        self.PROCESSING_DIR = Path(system_config.get('processing_directory', 'processing'))
        self.LOG_DIR = Path(system_config.get('log_directory', 'logs'))
        self.BACKUP_DIR = Path(system_config.get('backup_directory', 'backup'))
        self.METRICS_FILE = Path(system_config.get('metrics_file', 'system_metrics.json'))

        keep_alive_config = self.config_data.get('keep_alive', {})
        self.HEARTBEAT_FILE = Path(keep_alive_config.get('heartbeat_file', 'system_heartbeat.json'))
        self.KEEP_ALIVE_INTERVAL = keep_alive_config.get('interval_seconds', 10)

        processing_config = self.config_data.get('processing', {})
        self.MAX_WORKERS = processing_config.get('max_workers', multiprocessing.cpu_count())

        # Create directories if they don't exist
        for d in [self.WATCH_DIR, self.OUTPUT_DIR, self.PROCESSING_DIR, self.LOG_DIR, self.BACKUP_DIR]:
...
class MetricsManager:
    ...
    def get_summary(self):
        ...
        summary += f"Successful Organizations: {self.stats['successful_organizations']}\n"
...
class MemoryBank:
    ...
    def get_classification(self, filename):
        try:
            self.cursor.execute("SELECT category FROM classification_history WHERE filename = ?", (filename,))
            result = self.cursor.fetchone()
            if result:
                logger.debug(f"🧠 Found classification for '{filename}' in memory: '{result[0]}'")
                return result[0]
            return None
        except sqlite3.Error as e:
...
class DocumentProcessor:
    ...
    def get_legal_document_type(self, text, filename):
        ...
        if any(word in text_lower for word in ['motion to', 'plaintiff', 'defendant', 'court order']):
            return "Court_Documents/Motions"
        elif any(word in text_lower for word in ['agreement', 'contract', 'terms and conditions']):
...
class LegalOrganizer:
    ...
    def move_file(self, source_path, category):
        try:
            ...
            new_name = self._generate_intelligent_filename(text_content, source_path.name)
...
def process_file_worker(args):
    ...
    try:
        ...
        print(f"[WORKER ERROR] Failed to process {file_path.name}: {e}") # Use print as logger may not be configured in child process
        end_time = time.time()
...
class ContinuousDocumentOrganizer:
        ...
        while self.running:
            try:
                heartbeat_data = {
                    ...
                    'status': 'active',
                    'processed_files': self.metrics_manager.stats.get('total_files_processed', 0)
                }
    ...
    def process_single_file(self, file_path: Path):
        ...
        try:
            ...
            category = self.processor.get_legal_document_type(text_content, processing_path.name)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if files_to_process:
            logger.info(f"Found {len(files_to_process)} existing files. Processing in parallel...")
            ...
            for result in results:
                ...
                logger.info(f"Startup result for '{filename}': Category='{category}', Time={proc_time:.2f}s")
        ...
        organizer_system = ContinuousDocumentOrganizer(config, memory_bank, metrics_manager)
