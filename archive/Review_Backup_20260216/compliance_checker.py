...
class ComplianceChecker(BaseAgent, MemoryEnabledMixin):
    def __init__(self, services: ProductionServiceContainer, config: Optional[ComplianceCheckConfig] = None):
        super().__init__(services, agent_name="ComplianceChecker", agent_type="compliance_checking", timeout_seconds=120.0)
    ...
    async def _process_task(self, task_data: Any, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            await self.store_analysis_result("compliance", document_id, result, confidence_score=result["confidence"], metadata=metadata)
        except Exception:
            pass
        return result


async def create_compliance_checker(services: ProductionServiceContainer, config: Optional[ComplianceCheckConfig] = None) -> ComplianceChecker:
