...
class JsonLogFormatter(LogFormatter):
    ...
    def format(self, record: LogRecord) -> str:  # noqa: C901
        ...
        try:
            ...
            return json.dumps(
                log_data, indent=self.indent, default=self._json_serializer
            )
...
class HumanReadableLogFormatter(LogFormatter):
    ...
    def format(self, record: LogRecord) -> str:  # noqa: C901
        ...
        try:
            ...
            # Timestamp
            if self.include_timestamp:
                if self.timestamp_format.endswith(".%"):
            ...
            for key, value in context.items():
                if isinstance(value, dict):
                    ...
                    for sub_key, sub_value in value.items():
                        lines.append(
                            f"{' ' * (self.context_indent + 4)}{sub_key}: {sub_value}"
                        )
                else:
    ...
    def _format_exception(self, exception: Exception) -> list[str]:
        ...
        lines.append(
            f"{' ' * (self.context_indent + 2)}{type(exception).__name__}: {exception}"
        )
    ...
    def format(self, record: LogRecord) -> str:  # noqa: C901
        ...
        try:
            ...
            if self.include_timestamp:
                ...
                timestamp_str = record.timestamp.strftime("%H:%M:%S.%")[:-3]
                parts.append(timestamp_str)
            ...
            raise LogFormatterError(
                f"Failed to format log record in compact format: {e}"
            )
        ...
        try:
            ...
            if record.context:
                for key, value in record.context.items():
                    ...
                    escaped_value = (
                        str(value)
                        .replace(self.field_separator, "\\|")
                        .replace(self.kv_separator, "\\=")
                    )
            ...
            if record.exception:
                fields.append(
                    f"exception_type{self.kv_separator}{type(record.exception).__name__}"
                )
                fields.append(
                    f"exception_message{self.kv_separator}{str(record.exception)}"
                )
            ...
            raise LogFormatterError(
                f"Failed to format log record as structured text: {e}"
            )
...
class DebugLogFormatter(LogFormatter):
    ...
    def format(self, record: LogRecord) -> str:  # noqa: C901
        ...
        try:
            ...
            # Header line
            header = (
                f"=== LOG RECORD [{record.level.name}] [{record.category.value}] ==="
            )
            lines.append(header)
