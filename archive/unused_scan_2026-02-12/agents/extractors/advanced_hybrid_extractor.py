...
try:
    ...
    from transformers import AutoModelForTokenClassification, AutoTokenizer, pipeline  # noqa: E402
...
class HybridExtractorAgent(BaseAgent):
    ...
    async def _process_task(
        ...
        try:
            ...
            result = await self.extract_entities_hygrid(
                text, entity_types, confidence_threshold
            )
    ...
    async def _store_extraction_results(
        self, result: "HygridExtractionResult", context: str
    ):
        ...
        try:
            ...
            await self.memory_service.store_memory(
                "hybrid_extraction_results", memory_data
            )
        except Exception as e:
...
class LegacyHygridExtractorAgent:
    ...
    async def _init_transformer_models(self):
        if not TRANSFORMERS_AVAILABLE:
            logger.warning(
                "Transformers not available, skipping transformer model initialization"
            )
            return
    ...
    async def _extract_with_tfidf_similarity(  # noqa: C901
        self, text: str, entity_types: Optional[List[str]] = None
    ) -> ExtractionResult:
        start_time = datetime.now(timezone.utc)
        entities = []
        ...
        try:
            legal_terms = {
                ...
                "legal_document": ["complaint", "motion", "brie", "order", "judgment"],
                "legal_party": ["plainti", "defendant", "petitioner", "respondent"],
