...
class Action:
    ...
    def do(self, args, commands):
        if self.type == ActionType.COPY:
            ...
            Action.uvpip(self.args[0], args, self.args[2] if len(self.args) > 2 else None)
        elif self.type == ActionType.DOWNLOAD:
            Action.download(self.args[0], args)

    @staticmethod
    def uvpip(cmd: str, args: Any, env_vars: str | None = None):
        logger.info(cmd)
        ...
        if env_vars:
            ...
            result = subprocess.run(commands, text=True, env={**os.environ, **env_vars_dict})
        else:
    ...
    def copy(src: str, dst: str, args: Any):
        ...
        for file in glob.glob(src):
            ...
            try:
                ...
                logger.error(f"Failed to copy {file} to {dst}, please fix by releasing process lock etc.")
                raise
