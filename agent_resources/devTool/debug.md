...
Besides, for agent as HTTP server, introduce `agentdev` tool, fully integrated with AI Toolkit Agent Inspector for interactive debugging and testing, supporting:
...
- (REQUIRED) HTTP-Serverized, i.e., using azure.ai.agentserver.agentframework SDK. If not, **IMPORTANT** call tool `aitk-agent_as_server` to get best practices of HTTP server mode.
...
```json
// .vscode/tasks.json - run agent with debugging enabled
// Note - no need to install dependencies via task (users may have their own python env)
{
  "version": "2.0.0",
  "tasks": [
    {
      "label": "Validate prerequisites",
      // this is an AI Toolkit built-in task to check port occupancy. The task definition has fixed type and command names, fixed args schema, but the port numbers list can be customized
      "type": "aitk",
    ...
    },
    {
      ...
      "dependsOn": [
        // exactly match above task label
        "Run Agent/Workflow HTTP Server"
      ],
    },
    {
      ...
      "type": "shell",
      // use `${command:python.interpreterPath}` to point to current user's python env
      "command": "${command:python.interpreterPath} -m debugpy --listen 127.0.0.1:5679 -m agentdev run <your_entrypoint.py> --cli",
      "isBackground": true,
      "options": {
        "cwd": "${workspaceFolder}"
      },
...
```json
// .vscode/launch.json - attach debugger to the running agent/workflow
{
    "version": "0.2.0",
    "configurations": [
        {
