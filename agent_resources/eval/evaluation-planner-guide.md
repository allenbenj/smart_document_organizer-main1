...
4. **Runtime** - Where the evaluation will run (Locally or in Microsoft Foundry)
...
```
🕵️‍♂️ These are required Information to setup evaluation
...
- **Runtime**: Where the evaluation will run (Locally or in Microsoft Foundry). You can change the runtime later.

I found the following evaluation information:
- Evaluation Metrics: [Any evaluation objectives mentioned or "❌ Missing - need to clarify"]
- Available Queries: [Query files/examples if found or "❌ Missing - need to clarify"]
- Available Responses: [Response files/data if found or "❌ Missing - need to clarify"]
- Runtime: [Runtime preference if mentioned or "❌ Missing - need to clarify"]
...
```
**❓ Should I proceed with the suggested evaluation metrics above?**

- ✅ Yes, use the suggested evaluation metrics (default)
- ❌ No, I'll describe my own evaluation metrics
...
**Echo confirmation:**
```
Perfect! ✅ I've noted your evaluation metrics:
- [Metric 1]: [Description]
- [Metric 2]: [Description]
...
- ✅ Yes, generate test queries for me (default)
- ❌ No, I have existing query files to use
...
**Echo confirmation:**
```
"Great! ✅ I've got your queries sorted:
...
**Ask for confirmation (MANDATORY USER CONSULTATION):**
```
** ❓Should I proceed to run your app to collect fresh responses?**

- ✅ Yes, run my app to collect responses (default)
- ❌ No, I have existing response files to use
...
**Process based on response:**
- If needs collection: Follow the "For agent runner collection" section below to collect responses
...
**Echo confirmation:**
```
"Excellent! ✅ Your responses are ready:
...
**Echo clarification header:**
```
## ⚙️ Clarifying Runtime Environment

Finally, I need to know where you want to run the evaluation.
```

**Tell user the way to provide information:**
```
You have two options for running the evaluation (you can change this later):
1. **Run locally**: Uses [`azure-ai-evaluation`](https://pypi.org/project/azure-ai-evaluation/) SDK. Good for quick testing and development.
2. **Run in Microsoft Foundry**: Uses [`azure-ai-projects`](https://pypi.org/project/azure-ai-projects/2.0.0b2/) SDK. Better for scalable, managed evaluations in the cloud.
```

**Ask for confirmation (MANDATORY USER CONSULTATION):**

**Scenario A: If "Foundry Project Available" is "Yes" in Context:**
```
**❓ Should I run the evaluation in Microsoft Foundry using project [Active Foundry Project Name]?**

- ✅ Yes, run in Microsoft Foundry (default)
- ❌ No, run locally

I found an active Foundry project: **[Active Foundry Project Name]**.
Let me know if you'd like to use it or run locally!

> **Note:** You can always change the Foundry project endpoint in the generated code or `.env` file later.
```

**Scenario B: If "Foundry Project Available" is "No" in Context:**
```
**❓ Should I run the evaluation locally?**

- ✅ Yes, run locally (default)
- ☁️ No, run in Microsoft Foundry
...
**Process based on response:**
- If local: Set runtime to local (azure-ai-evaluation)
- If Foundry: Set runtime to Foundry (azure-ai-projects)

**Echo confirmation:**
```
"Got it! ✅ Runtime environment set:
- Environment: [Local or Microsoft Foundry]
- SDK: [azure-ai-evaluation or azure-ai-projects]
Runtime clarified!"
```
...
```
"Awesome! 🎯 I've got everything needed for your evaluation setup:
...
- **Runtime:** [Local or Microsoft Foundry]
- **SDK:** [azure-ai-evaluation or azure-ai-projects]
...
```
"Perfect! ✅ All evaluation details are locked in. 
...
- **Examples of targeted responses**:
  - User: "evaluate tool call accuracy" → Suggest only ToolCallAccuracyEvaluator (1 metric)
  - User: "measure response relevance" → Suggest only RelevanceEvaluator (1 metric)
  - User: "check if responses are coherent" → Suggest only CoherenceEvaluator (1 metric)
  - User: "evaluate my RAG system" → Suggest GroundednessEvaluator + RelevanceEvaluator (2 metrics max)
  - User: "evaluate accuracy, relevance, coherence, fluency" → Suggest 4 metrics only because user specified 4 objectives

### Clarification Process Requirements
**🚨 CRITICAL: AI MUST follow ALL steps in sequence - NO EXCEPTIONS:**
...
**⚠️ WARNING: Skipping Steps 1 or 2 breaks the user experience and violates the clarification process!**
...
- ✅ Specific evaluation metrics are identified (what to evaluate)
- ✅ Queries are available (input examples)
- ✅ Responses are available (expected/actual outputs)
